...
class CovidDataController extends Controller{
    ...
    public function latest_covid_data(Request $request){
        ...
        if ($html == false){
            $response = [
                "message" => "Data not found, Please Check your network connection!!!",
            ];
            return $response;
        }
    ...
    /**
     * @OA\Get(
     *     path="/top_twenty_covid_case",
     *     operationId="/top_twenty_covid_case",
     *     tags={"Covid Data"},
     *     @OA\Response(
     *         response="200",
     ...
     * )
     */
    public function top_twenty_covid_case(){
    ...
    private function get_web_page($url = '', $option = array(), $getinfo = false){
        ...
        $data['error'] = curl_error($curl); // get error message occured
